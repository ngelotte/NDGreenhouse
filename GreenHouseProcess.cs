...
namespace NdGreenhouse.Apps.Greenhouse
{
    ...
    public class GhProcedures
    {
        ...
        DateTime? lastTimetheHvacWasOpen;
        ...
        public async Task<bool> HeatCurrentZone()
        {
            ...
            InputBooleanEntities ibe = new(_app);
            try
            {
                if (_ghMain != null && _ghMain.TestingZoneMedium != null)
                {
                    //using EnsurePumpTurnsOff testingHeater = new(_ghMain.TestingTankHeater);

                    ibe.TestingTankHold.TurnOn();
                    var isTheholdWorking = await _ghMain.TestingZoneMedium.StateAllChanges().Where(t => t.New.IsOn()).FirstOrTimeoutAsync(TimeSpan.FromSeconds(30));
                    if (isTheholdWorking == null)
                    {
                        SendAlert("Tank heating has failed", "Trying to heat the water but it looks like the hold is not working");
                        return false;
                    }
                    _ghMain.TestingTankHeater.TurnOn();
                    var heatingWaitResult = await _ghMain.TestingTankTemp.StateChanges().Where(t => t?.New?.State > 65).FirstOrTimeoutAsync(TimeSpan.FromMinutes(20));
                    if (heatingWaitResult == null)
                    {
                        SendAlert("Could not heat the water to full temp in 15 minutes.", $"The temp reached {_ghMain.TestingTankTemp.AsNumeric().State} instead");
                    }
                    else
                    {
                        logger.LogInformation("Zone heated sucesfully");
                    }
                    _ghMain.TestingTankHeater.TurnOff();
                    ibe.TestingTankHold.TurnOff();
            ...
            catch (Exception ex)
            {
                ...
                SendAlert("Error in Heating zone", ex.Message);
        ...
        public async Task<bool> RunOneTankEmptyRunForCurrentZone()
        {
            ...
            {
                using EnsurePumpTurnsOff testingPump = new(zone.TestingPump);
                testingPump.TurnOn();
                ...
                else
                {
                    ...
                    {
                        dumpPump.TurnOn();
                        var refill = await zone.LowWater.StateChanges().Where(t => t.New.IsOn()).Merge(_ghMain.TestingZoneHigh.StateChanges().Where(t => t.New.IsOn())).FirstOrTimeout(TimeSpan.FromSeconds(80));
                        dumpPump.TurnOff();
                        if (refill.New == null)
                        {
        ...
        public async Task<bool> AddNutrientsToCurrentZone()
        {
            ...
            int numberOfUnits = (int)inputNumberEntities.Nutrientunitstoadd.State;
        ...
        public bool SendAlert(string title, string description)
        {
            ...
            _app.CallService("notify", "mobile_app_samsungflex", data: new
            {
                title = title,
                message = description
            });
